...
namespace CBS.SystemConfiguration.MediatR.Handlers
{
    ...
    /// </summary>
    public class GetAllSubdivisionByDivisionIdQueryHandler : IRequestHandler<GetAllSubdivisionByDivisionIdQuery, ServiceResponse<List<SubdivisionDto>>>
    {
        private readonly ISubdivisionRepository _SubdivisionRepository; // Repository for accessing OperationEvent data.
        private readonly IMapper _mapper; // AutoMapper for object mapping.
        private readonly ILogger<GetAllSubdivisionByDivisionIdQueryHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public GetAllSubdivisionByDivisionIdQueryHandler(
            ISubdivisionRepository SubdivisionRepository,
            IMapper mapper, ILogger<GetAllSubdivisionByDivisionIdQueryHandler> logger)
        {
        ...
        public async Task<ServiceResponse<List<SubdivisionDto>>> Handle(GetAllSubdivisionByDivisionIdQuery request, CancellationToken cancellationToken)
        {
            try
            {
                // Retrieve all OperationEvent entities from the repository
                var entities = await _SubdivisionRepository.All.Where(x=>x.IsDeleted.Equals(false)&&x.DivisionId==request.Id    ).ToListAsync();
                // Retrieve all OperationEvent entities from the repository
                if (entities.Any())
                {
...

                {
                    ...
                    _logger.LogError("Subdivisions not found.");
                    return ServiceResponse<List<SubdivisionDto>>.Return404();
                }
