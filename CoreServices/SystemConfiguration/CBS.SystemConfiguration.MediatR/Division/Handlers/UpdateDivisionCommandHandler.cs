...
namespace CBS.SystemConfiguration.MediatR.Handlers
{
    ...
    public class UpdateDivisionCommandHandler : IRequestHandler<UpdateDivisionCommand, ServiceResponse<DivisionDto>>
    {
        ...
        private readonly ILogger<UpdateDivisionCommandHandler> _loggger; // Logger for logging handler actions and errors.
        private readonly IMapper _mapperr;  // AutoMapper for object mapping.
        private readonly IUnitOfWork<SystemContext> _unitOfWork;
        ...
        public async Task<ServiceResponse<DivisionDto>> Handle(UpdateDivisionCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (existingDivision != null)
                {
                    ...
                    existingDivision = _mapperr.Map(request, existingDivision);
                    // Use the repository to update the existing Division entity
                    _DivisionRepository.Update(existingDivision);
                    await _unitOfWork.SaveAsync();
                    // Prepare the response and return a successful response with 200 status code
                    var response = ServiceResponse<DivisionDto>.ReturnResultWith200(_mapperr.Map<DivisionDto>(existingDivision));
                    _loggger.LogInformation($"Division {request.Id} was successfully updated.");
                    return response;
                }
                else
                {
                    ...
                    _loggger.LogError(errorMessage);
