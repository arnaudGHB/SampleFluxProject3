...
{
    public class CashMovementDenominationCommandHandler : IRequestHandler<CashMovementDenominationCommand, ServiceResponse<bool>>
    {
  
        private readonly IMapper _mapper;
        private readonly ILogger<CashMovementDenominationCommandHandler> _logger;
        ...
        public CashMovementDenominationCommandHandler(
    ...
    IAccountingRuleEntryRepository accountingRuleEntryRepository,
            ...
            ILogger<CashMovementDenominationCommandHandler> logger,
        ...
        public async Task<ServiceResponse<bool>> Handle(CashMovementDenominationCommand request, CancellationToken cancellationToken)
        {
            try
            {
                string strigifyData = JsonConvert.SerializeObject(request);
                var customerData = await APICallHelper.PostCashInOrCashdenominationOperation<ServiceResponse<bool>>(_pathHelper, _userInfoToken.Token, strigifyData,  _pathHelper.CashInOrCashOutDenomination);
                await BaseUtilities.LogAndAuditAsync($"Posting {request.OperationType } Denomination", request, HttpStatusCodeEnum.OK, LogAction.DenominationPosting, LogLevelInfo.Information);
                return ServiceResponse<bool>.ReturnResultWith200(customerData.Data, "Accounting posting was successfull");

            }
            catch (Exception ex)
            {
                await BaseUtilities.LogAndAuditAsync($"Posting {request.OperationType}  Denomination", request, HttpStatusCodeEnum.InternalServerError, LogAction.DenominationPosting, LogLevelInfo.Error);
