...
namespace CBS.AccountManagement.MediatR.Handlers
{
    ...
    /// </summary>
    public class GetDocumentReferenceCodeQueryHandler : IRequestHandler<GetDocumentReferenceCodeQuery, ServiceResponse<DocumentReferenceCodeDto>>
    {
        ...
        private readonly ILogger<GetDocumentReferenceCodeQuery> _logger; // Logger for logging handler actions and errors.
        ...
        public GetDocumentReferenceCodeQueryHandler(
            IDocumentReferenceCodeRepository documentReferenceCodeRepository,
            IMapper mapper, ILogger<GetDocumentReferenceCodeQuery> logger, IDocumentRepository? documentRepository, IDocumentTypeRepository? documentTypeRepository)
        {
        ...
        public async Task<ServiceResponse<DocumentReferenceCodeDto>> Handle(GetDocumentReferenceCodeQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                return ServiceResponse<DocumentReferenceCodeDto>.ReturnResultWith200(_mapper.Map<DocumentReferenceCodeDto>(result.Find(x=>x.Id==request.Id)));
            }
            catch (Exception e)
            {
                // Log error and return a 500 Internal Server Error response with error message
                _logger.LogError($"Failed to get all DocumentDto: {e.Message}");
                return ServiceResponse<DocumentReferenceCodeDto>.Return500(e, "Failed to get all DocumentDto");
            }
        }
    }
}