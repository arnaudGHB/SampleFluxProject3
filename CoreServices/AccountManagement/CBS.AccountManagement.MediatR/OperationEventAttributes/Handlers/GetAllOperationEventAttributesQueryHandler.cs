...
namespace CBS.AccountManagement.MediatR.Handlers
{
    ...
    /// </summary>
    public class GetAllOperationEventAttributesQueryHandler : IRequestHandler<GetAllOperationEventAttributesQuery, ServiceResponse<List<OperationEventAttributesDto>>>
    {
        ...
        public GetAllOperationEventAttributesQueryHandler(
        ...
        public async Task<ServiceResponse<List<OperationEventAttributesDto>>> Handle(GetAllOperationEventAttributesQuery request, CancellationToken cancellationToken)
        {
            try
            {
                // Retrieve all OperationEventAttributess entities from the repository
                var entities = await _OperationEventAttributesRepository.All.Where(x=>x.IsDeleted==false).ToListAsync();
                // Write the JSON to a file
                File.WriteAllText("OperationEventAttributes.json", JsonConvert.SerializeObject(_mapper.Map<List<OperationEventAttributesDto>>(entities)));
