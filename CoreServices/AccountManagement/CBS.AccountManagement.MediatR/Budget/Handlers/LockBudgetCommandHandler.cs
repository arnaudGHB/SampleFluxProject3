...
namespace CBS.AccountManagement.MediatR.Handlers
{
    ...
    /// </summary>
    public class LockBudgetCommandHandler : IRequestHandler<LockBudgetCommand, ServiceResponse<BudgetDto>>
    {
        private readonly IBudgetRepository _BudgetNameRepository; // Repository for accessing BudgetName data.
        private readonly ILogger<LockBudgetCommandHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public LockBudgetCommandHandler(
            IBudgetRepository BudgetNameRepository,
            ILogger<LockBudgetCommandHandler> logger,
            IMapper mapper,
        ...
        public async Task<ServiceResponse<BudgetDto>> Handle(LockBudgetCommand request, CancellationToken cancellationToken)
        {
           string errorMessage = string.Empty;
            try
            {
                ...
                if (existingBudgetName != null)
                {
                    ...
                    {
                        errorMessage = $"Budget {request.Id} was successfully already LOCKED.";
                        // Prepare the response and return a successful response with 200 status code
                      
                        _logger.LogInformation(errorMessage);
                        await APICallHelper.AuditLogger(_userInfoToken.Email, "LockBudgetCommand",
                    ...
                    existingBudgetName.IsLocked = true;
                    existingBudgetName.LockDate =BaseUtilities.UtcToDoualaTime( DateTime.Now);
                    existingBudgetName.LockedBy = _userInfoToken.Id;
                    _BudgetNameRepository.Update(existingBudgetName);
                    await _uow.SaveAsync();

                    errorMessage = $"Budget {request.Id} was successfully LOCKED.";
                    // Prepare the response and return a successful response with 200 status code
