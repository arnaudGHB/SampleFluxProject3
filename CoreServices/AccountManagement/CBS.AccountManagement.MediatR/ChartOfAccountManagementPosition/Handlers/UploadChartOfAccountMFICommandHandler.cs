...
namespace CBS.AccountManagement.MediatR.Handlers
{
    ...
    public class UploadChartOfAccountMFICommandHandler : IRequestHandler<UploadChartOfAccountMFICommand, ServiceResponse<bool>>
    {
        // Dependencies
        ...
        public async Task<ServiceResponse<bool>> Handle(UploadChartOfAccountMFICommand request, CancellationToken cancellationToken)
        {
            List<Data.ChartOfAccountManagementPosition> modelList = _chartOfAccountManagementPositionRepository.All.ToList(); 
            List<Data.Account> accounts = new List<Data.Account>();
            List<Data.ChartOfAccountManagementPosition> accountinges = new List<Data.ChartOfAccountManagementPosition>();
            ...
            List<ChartOfAccountManagementPositionFile2> accountsNotPresent = new List<ChartOfAccountManagementPositionFile2>();
            List<Data.ChartOfAccount> ListOfchartOfAccounts = _ChartOfAccountRepository.All.ToList();
            Data.ChartOfAccountManagementPosition modelx = new Data.ChartOfAccountManagementPosition();
            List<Data.ChartOfAccountManagementPosition>  List = new List<Data.ChartOfAccountManagementPosition>();
            bool modelEx451Ispresent = false;
            try
            {
                foreach (var item in request.ChartOfAccountManagementPositionFile)
                {
                  var model=  modelList.Where(x=> x.Old_AccountNumber.Trim() == item.AccountNumber.Trim());   
                    if (model.Any())
                    {
                   
                        List.Add(model.FirstOrDefault());
                    }
                    else
                    {
                        accountsNotPresent.Add(item);

                        chartOfAccount = await CreateAccountAsync(item.AccountNumber, item.AccountName, item.AccountName, false);
                        modelx = CreateChartOfAccountManagementPosition(chartOfAccount, item, item.AccountNumber, List, item.AccountNumber, false);

                        chartOfAccounts.Add(chartOfAccount);
                        accountinges.Add(modelx);
                    }

                }
                _ChartOfAccountRepository.AddRange(chartOfAccounts);
                _chartOfAccountManagementPositionRepository.AddRange(accountinges);
    ...
    List<ChartOfAccountDto> result)
        ...
        private Data.ChartOfAccountManagementPosition CreateChartOfAccountManagementPosition(Data.ChartOfAccount chartOfAccount, ChartOfAccountManagementPositionFile2 item, string accountNumber, List<ChartOfAccountManagementPosition> modelList, string old_accountNumber,bool IsUniqueAccount)
        {
