...
namespace CBS.AccountManagement.MediatR.Handlers
{
    ...
    /// </summary>
    public class DeleteTrailBalanceUploudCommandHandler : IRequestHandler<DeleteTrailBalanceUploudCommand, ServiceResponse<bool>>
    {
        private readonly ITrailBalanceUploudRepository _trailBalanceUploudRepository; // Repository for accessing BudgetName data.
        private readonly ILogger<DeleteTrailBalanceUploudCommandHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public DeleteTrailBalanceUploudCommandHandler(
            ITrailBalanceUploudRepository BudgetRepository, IMapper mapper,
            ILogger<DeleteTrailBalanceUploudCommandHandler> logger
, IUnitOfWork<POSContext> uow, UserInfoToken? userInfoToken)
        ...
        public async Task<ServiceResponse<bool>> Handle(DeleteTrailBalanceUploudCommand request, CancellationToken cancellationToken)
        {
            string errorMessage = null;
            try
            {
                // Check if the BudgetName entity with the specified ID exists
                var existingBudgetName = await _trailBalanceUploudRepository.FindAsync(request.Id);
                if (existingBudgetName == null)
                {
                    errorMessage = $"TrailBalanceUploud with ID {request.Id} not found.";
            ...
            request, errorMessage, LogLevelInfo.Information.ToString(), 404, _userInfoToken.Token);
                ...
                _trailBalanceUploudRepository.Update(existingBudgetName);
                errorMessage = $"TrailBalanceUploud with ID {request.Id} has been deleted successfully.";
                await APICallHelper.AuditLogger(_userInfoToken.Email, "DeleteBudgetCommand",
