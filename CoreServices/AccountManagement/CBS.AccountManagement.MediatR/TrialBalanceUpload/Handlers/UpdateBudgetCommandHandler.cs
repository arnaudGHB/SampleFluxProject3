...
namespace CBS.AccountManagement.MediatR.Handlers
{
    ...
    /// </summary>
    public class UpdateTrailBalanceUploudCommandHandler : IRequestHandler<UpdateTrailBalanceUploudCommand, ServiceResponse<TrailBalanceUploudDto>>
    {
        private readonly ITrailBalanceUploudRepository _BudgetNameRepository; // Repository for accessing BudgetName data.
        private readonly ILogger<UpdateTrailBalanceUploudCommandHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public UpdateTrailBalanceUploudCommandHandler(
            ITrailBalanceUploudRepository BudgetNameRepository,
            ILogger<UpdateTrailBalanceUploudCommandHandler> logger,
            IMapper mapper,
        ...
        public async Task<ServiceResponse<TrailBalanceUploudDto>> Handle(UpdateTrailBalanceUploudCommand request, CancellationToken cancellationToken)
        {
           string errorMessage = string.Empty;
            try
            {
                ...
                if (existingBudgetName != null)
                {
                    ...
                    existingBudgetName= _mapper.Map(request, existingBudgetName);
                    // Use the repository to update the existing BudgetName entity
                    _BudgetNameRepository.Update(existingBudgetName);
                    await _uow.SaveAsync();

                    errorMessage = $"Budget {request.Id} was successfully updated.";
                    // Prepare the response and return a successful response with 200 status code
                    var response = ServiceResponse<TrailBalanceUploudDto>.ReturnResultWith200(_mapper.Map<TrailBalanceUploudDto>(existingBudgetName));
                ...
                request, errorMessage, LogLevelInfo.Information.ToString(), 404, _userInfoToken.Token);
                    ...
                    return ServiceResponse<TrailBalanceUploudDto>.Return404(errorMessage);
                }
            }
            catch (Exception e)
            {
