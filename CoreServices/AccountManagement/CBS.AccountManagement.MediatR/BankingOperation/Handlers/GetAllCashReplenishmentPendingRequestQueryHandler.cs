...
{
    public class GetAllCashReplenishmentPendingRequestQueryHandler : IRequestHandler<GetAllCashReplenishmentPendingRequestQuery, ServiceResponse<List<CashReplenishmentDto>>>
    {
        ...
        private readonly ILogger<GetAllCashReplenishmentPendingRequestQueryHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public GetAllCashReplenishmentPendingRequestQueryHandler(
            ICashReplenishmentRepository cashReplenishmentRepository,
            IMapper mapper,
            ILogger<GetAllCashReplenishmentPendingRequestQueryHandler> logger, UserInfoToken userInfoToken)
        {
        ...
        public async Task<ServiceResponse<List<CashReplenishmentDto>>> Handle(GetAllCashReplenishmentPendingRequestQuery request, CancellationToken cancellationToken)
        {
          ...
          string errorMessage = null;
            try
            {
                ...
                else 
                {
                    ...
                    entitiesAccounts = await _cashReplenishmentRepository.All.Where(x => x.CorrespondingBranchId.Equals(_userInfoToken.BranchId) && x.IsDeleted.Equals(false) &&x.CashReplishmentRequestStatus.ToLower().Equals(CashReplishmentRequestStatus.Pending.ToString().ToLower())).ToListAsync();
                ...
                if (entitiesAccounts == null)
                {
                    ...
                    await BaseUtilities.LogAndAuditAsync(errorMessage, request, HttpStatusCodeEnum.NotFound, LogAction.GetAllCashReplenishmentPendingRequestQuery, LogLevelInfo.Warning);
                ...
                else
                {
                  _listMpped = _mapper.Map(entitiesAccounts, _listMpped);
                    await BaseUtilities.LogAndAuditAsync(errorMessage, request, HttpStatusCodeEnum.OK, LogAction.GetAllCashReplenishmentPendingRequestQuery, LogLevelInfo.Information);
