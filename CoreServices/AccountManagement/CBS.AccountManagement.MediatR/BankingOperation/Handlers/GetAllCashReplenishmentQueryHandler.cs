...
{
    public class GetAllCashReplenishmentQueryHandler  : IRequestHandler<GetAllCashReplenishmentQuery, ServiceResponse<List<CashReplenishmentDto>>>
    {
        ...
        public GetAllCashReplenishmentQueryHandler(
            ...
            IMapper mapper,
            ILogger<GetAllCashReplenishmentQueryHandler> logger, UserInfoToken userInfoToken)
        {
        ...
        public async Task<ServiceResponse<List<CashReplenishmentDto>>> Handle(GetAllCashReplenishmentQuery request, CancellationToken cancellationToken)
        {
          ...
          string errorMessage = null;
            try
            {
                ...
                else
                {
                    ...
                    entitiesAccounts = await _cashReplenishmentRepository.All.Where(x => x.BranchId.Equals(_userInfoToken.BranchId) && x.IsDeleted.Equals(false)|| x.CorrespondingBranchId.Equals(_userInfoToken.BranchId)).ToListAsync();
                ...
                if (entitiesAccounts == null)
                {
                    errorMessage = $"No cash replenishment request was found";
                           ...
                           await BaseUtilities.LogAndAuditAsync(errorMessage, request, HttpStatusCodeEnum.NotFound, LogAction.GetAllCashReplenishmentQuery, LogLevelInfo.Error);
                ...
                else
                {
                  _listMpped = _mapper.Map(entitiesAccounts, _listMpped);
                    errorMessage = $"GetAllCashReplenishmentQuery request was successfull with {_listMpped.Count()}";
                    await BaseUtilities.LogAndAuditAsync(errorMessage, request, HttpStatusCodeEnum.OK, LogAction.GetAllCashReplenishmentQuery, LogLevelInfo.Information);
            ...
            catch (Exception e)
            {
                errorMessage = $"Error occurred while getting Account: {e.Message}";
                       await BaseUtilities.LogAndAuditAsync(errorMessage, request, HttpStatusCodeEnum.InternalServerError, LogAction.GetAllCashReplenishmentQuery, LogLevelInfo.Error);
    ...
    public class GetAllCashReplenishmentQueryAsBranchHandler : IRequestHandler<GetAllCashReplenishmentQueryAsBranch, ServiceResponse<List<CashReplenishmentDto>>>
    {
        private readonly ICashReplenishmentRepository _cashReplenishmentRepository; // Repository for accessing Account data.
        private readonly IMapper _mapper; // AutoMapper for object mapping.
        private readonly ILogger<GetAllCashReplenishmentQueryAsBranchHandler> _logger; // Logger for logging handler actions and errors.
        private readonly UserInfoToken _userInfoToken;
        ...
        /// <param name="claimTypeRepository">Repository for ClaimType data access.</param>
        /// <param name="logger">Logger for logging handler actions and errors.</param>
        public GetAllCashReplenishmentQueryAsBranchHandler(
            ICashReplenishmentRepository cashReplenishmentRepository,
            IMapper mapper,
            ILogger<GetAllCashReplenishmentQueryAsBranchHandler> logger, UserInfoToken userInfoToken)
        {
            _cashReplenishmentRepository = cashReplenishmentRepository;
            _mapper = mapper;
            _logger = logger;
        ...
        /// <param name="request">The GetAccountQuery containing Account ID to be retrieved.</param>
        /// <param name="cancellationToken">A cancellation token.</param>
        public async Task<ServiceResponse<List<CashReplenishmentDto>>> Handle(GetAllCashReplenishmentQueryAsBranch request, CancellationToken cancellationToken)
        {
            List<CashReplenishmentDto> _listMpped = new List<CashReplenishmentDto>();
            List<CashReplenishment> entitiesAccounts = new List<CashReplenishment>();
            string errorMessage = null;
            try
            {
                ...
                else
                {
                    ...
                    {
                        entitiesAccounts = await _cashReplenishmentRepository.All.Where(x => x.BranchId.Equals(_userInfoToken.BranchId) && x.IsDeleted.Equals(false)).ToListAsync();
                    }
