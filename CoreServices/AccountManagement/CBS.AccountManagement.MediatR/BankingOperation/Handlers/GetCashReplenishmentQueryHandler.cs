...
{
    public class GetCashReplenishmentQueryHandler : IRequestHandler<GetCashReplenishmentQuery, ServiceResponse<CashReplenishmentDto>>
    {
        ...
        public GetCashReplenishmentQueryHandler(
            ...
            IMapper mapper,
            ILogger<GetCashReplenishmentQueryHandler> logger, UserInfoToken userInfoToken)
        {
        ...
        public async Task<ServiceResponse<CashReplenishmentDto>> Handle(GetCashReplenishmentQuery request, CancellationToken cancellationToken)
        {
            string errorMessage = null;
            try
            {
                // Retrieve the Account entity with the specified ID from the repository
                var entitiesAccounts = await _cashReplenishmentRepository.FindAsync(request.ReferenceId);
                ...
                else
                {
                   var _mapped = _mapper.Map<CashReplenishmentDto>(entitiesAccounts);
                    _mapped.TempData = entitiesAccounts.TempData;
