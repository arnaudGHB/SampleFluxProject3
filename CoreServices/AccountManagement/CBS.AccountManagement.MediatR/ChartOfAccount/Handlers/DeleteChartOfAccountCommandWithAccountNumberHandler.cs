...
namespace CBS.AccountManagement.MediatR.Handlers
{
    ...
    /// </summary>
    public class DeleteChartOfAccountCommandWithAccountNumberHandler : IRequestHandler<DeleteChartOfAccountCommandWithAccountNumber, ServiceResponse<bool>>
    {
        private readonly IChartOfAccountRepository _ChartOfAccountRepository; // Repository for accessing Account data.
        private readonly ILogger<DeleteChartOfAccountCommandWithAccountNumberHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public DeleteChartOfAccountCommandWithAccountNumberHandler(
            IChartOfAccountRepository ChartOfAccountRepository, IMapper mapper,
            ILogger<DeleteChartOfAccountCommandWithAccountNumberHandler> logger
, IUnitOfWork<POSContext> uow)
        ...
        public async Task<ServiceResponse<bool>> Handle(DeleteChartOfAccountCommandWithAccountNumber request, CancellationToken cancellationToken)
        {
            string errorMessage = null;
            try
            {
                // Check if the Account entity with the specified ID exists
                var existingAccount =   _ChartOfAccountRepository.FindBy(x=>x.AccountNumber.Equals(request.Id)&&x.IsDeleted.Equals(false)).FirstOrDefault();
