...
namespace CBS.AccountManagement.MediatR.Handlers
{
    ...
    /// </summary>
    public class GetAccountByMFIBankAccountNumberQueryHandler : IRequestHandler<GetAccountByMFIBankAccountNumberQuery, ServiceResponse<List<AccountDto>>>
    {
        private readonly IAccountRepository _AccountRepository; // Repository for accessing Accounts data.
        private readonly IMapper _mapper; // AutoMapper for object mapping.
        private readonly ILogger<GetAccountByMFIBankAccountNumberQueryHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public GetAccountByMFIBankAccountNumberQueryHandler( UserInfoToken userInfoToken,
            IAccountRepository AccountRepository,
            IMapper mapper, ILogger<GetAccountByMFIBankAccountNumberQueryHandler> logger)
        {
        ...
        public async Task<ServiceResponse<List<AccountDto>>> Handle(GetAccountByMFIBankAccountNumberQuery request, CancellationToken cancellationToken)
        {
            try
            {
                // Retrieve all Accounts entities from the repository
                
                var entities = await _AccountRepository.All.Where(x => x.BranchId.Equals(request.BranchId) && x.IsDeleted.Equals(false) && x.Account2.StartsWith("56")).ToListAsync();
