...
{
    public class DailyCollectionMonthlyPayableEventCommandHandler : IRequestHandler<DailyCollectionMonthlyPayableEventCommand, ServiceResponse<bool>>
    {
        ...
        private readonly ILogger<DailyCollectionMonthlyPayableEventCommandHandler> _logger;
        ...
        public DailyCollectionMonthlyPayableEventCommandHandler(
    ...
    IAccountingRuleEntryRepository accountingRuleEntryRepository,
            ...
            ILogger<DailyCollectionMonthlyPayableEventCommandHandler> logger,
        ...
        public async Task<ServiceResponse<bool>> Handle(DailyCollectionMonthlyPayableEventCommand command, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (await _accountingService.TransactionExists(command.TransactionReferenceId))
                {
                    ...
                    await BaseUtilities.LogAndAuditAsync(errorMessage, command, HttpStatusCodeEnum.Conflict, LogAction.DailyCollectionMonthlyPayableEventCommand, LogLevelInfo.Warning);
                    return ServiceResponse<bool>.Return409(errorMessage);
                }

                // 🚀 Step 2: Retrieve the Expense Account
                var expenseAccount = await _accountingService.GetAccount(command.ExpenseEventCode, _userInfoToken.BranchId, _userInfoToken.BranchCode);
                if (expenseAccount == null)
                {
                    errorMessage = $"Expense account not found for EventCode: {command.ExpenseEventCode}.";
                    _logger.LogWarning(errorMessage);
                    await BaseUtilities.LogAndAuditAsync(errorMessage, command, HttpStatusCodeEnum.NotFound, LogAction.DailyCollectionMonthlyPayableEventCommand, LogLevelInfo.Warning);
                ...
                else
                {
                    ...
                    await BaseUtilities.LogAndAuditAsync(errorMessage, command, HttpStatusCodeEnum.Forbidden, LogAction.DailyCollectionMonthlyPayableEventCommand, LogLevelInfo.Warning);
                    return ServiceResponse<bool>.Return422(errorMessage);
                }

                // ✅ Step 6: Return success response
                string successMessage = "Daily collection monthly payable transaction completed successfully.";
                await BaseUtilities.LogAndAuditAsync(successMessage, command, HttpStatusCodeEnum.OK, LogAction.DailyCollectionMonthlyPayableEventCommand, LogLevelInfo.Information);
                return ServiceResponse<bool>.ReturnResultWith200(true, successMessage);
            }
            catch (Exception ex)
            {
                // 🚨 Handle unexpected errors
                errorMessage = $"Error during daily collection monthly payable transaction: {ex.Message}.";
                _logger.LogError(ex, errorMessage);
                await BaseUtilities.LogAndAuditAsync(errorMessage, command, HttpStatusCodeEnum.InternalServerError, LogAction.DailyCollectionMonthlyPayableEventCommand, LogLevelInfo.Critical);
