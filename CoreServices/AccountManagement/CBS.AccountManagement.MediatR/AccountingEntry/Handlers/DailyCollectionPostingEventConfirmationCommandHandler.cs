...
{
    public class DailyCollectionPostingEventConfirmationCommandHandler : IRequestHandler<DailyCollectionConfirmationPostingEventCommand, ServiceResponse<bool>>
    {
        ...
        private readonly ILogger<DailyCollectionPostingEventConfirmationCommandHandler> _logger;
        ...
        public DailyCollectionPostingEventConfirmationCommandHandler(
    ...
    IAccountingRuleEntryRepository accountingRuleEntryRepository,
            ...
            ILogger<DailyCollectionPostingEventConfirmationCommandHandler> logger,
        ...
        public async Task<ServiceResponse<bool>> Handle(DailyCollectionConfirmationPostingEventCommand command, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // 🚀 Step 2: Retrieve Daily Collector Account
                var dailyCollectorAccount = await _accountingService.GetAccount(command.DailyCollectorSource, _userInfoToken.BranchId, _userInfoToken.BranchCode);
                if (dailyCollectorAccount == null)
                {
                    errorMessage = $"Daily Collector Account not found for source: {command.DailyCollectorSource}.";
                ...
                // 🚀 Step 3: Retrieve Physical Teller Account
                var physicalTellerAccount = await _accountingService.GetAccount(command.PhysicalTellerDestination, _userInfoToken.BranchId, _userInfoToken.BranchCode);
                if (physicalTellerAccount == null)
                {
                    errorMessage = $"Physical Teller Account not found for destination: {command.PhysicalTellerDestination}.";
                    _logger.LogWarning(errorMessage);
                    await BaseUtilities.LogAndAuditAsync(errorMessage, command, HttpStatusCodeEnum.NotFound, LogAction.DailyCollectionPostingEventCommand, LogLevelInfo.Warning);
                    return ServiceResponse<bool>.Return404(errorMessage);
                }

                // 🚀 Step 4: Process Accounting Entry
                accountingEntriesDto.AddRange(await _accountingService.CashMovementAsync(
                    command.Naration,
                    command.MemberReference,
                    command.TransactionDate,
                    dailyCollectorAccount,
                    physicalTellerAccount,
                ...
                // 🚀 Step 5: Map and Save Accounting Entries
                ...
                else
                {
                    ...
                    await BaseUtilities.LogAndAuditAsync(errorMessage, command, HttpStatusCodeEnum.Forbidden, LogAction.DailyCollectionPostingEventCommand, LogLevelInfo.Warning);
                    return ServiceResponse<bool>.Return422(errorMessage);
                }

                // ✅ Step 7: Return success response
                string successMessage = "Daily collection confirmation posting transaction completed successfully.";
            ...
            catch (Exception ex)
            {
                ...
                errorMessage = $"Error during daily collection confirmation posting transaction: {ex.Message}.";
