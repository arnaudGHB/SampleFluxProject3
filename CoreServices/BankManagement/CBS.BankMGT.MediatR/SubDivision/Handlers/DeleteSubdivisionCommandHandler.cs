using AutoMapper;
using MediatR;
using Microsoft.Extensions.Logging;
using CBS.BankMGT.MediatR.Commands;
using CBS.BankMGT.Repository;
using CBS.BankMGT.Helper;
using CBS.BankMGT.Common.UnitOfWork;
using CBS.BankMGT.Domain;
using Microsoft.EntityFrameworkCore;

namespace CBS.BankMGT.MediatR.Handlers
{
    /// <summary>
    /// Handles the command to delete a Subdivision based on DeleteSubdivisionCommand.
    /// </summary>
    public class DeleteSubdivisionCommandHandler : IRequestHandler<DeleteSubdivisionCommand, ServiceResponse<bool>>
    {
        private readonly ISubdivisionRepository _SubdivisionRepository; // Repository for accessing Subdivision data.
        private readonly ILogger<DeleteSubdivisionCommandHandler> _logger; // Logger for logging handler actions and errors.
        private readonly IMapper _mapper; // AutoMapper for object mapping.
        private readonly IUnitOfWork<POSContext> _uow;

        /// <summary>
        /// Constructor for initializing the DeleteSubdivisionCommandHandler.
        /// </summary>
        /// <param name="SubdivisionRepository">Repository for Subdivision data access.</param>
        /// <param name="logger">Logger for logging handler actions and errors.</param>
        public DeleteSubdivisionCommandHandler(
            ISubdivisionRepository SubdivisionRepository, IMapper mapper,
            ILogger<DeleteSubdivisionCommandHandler> logger
, IUnitOfWork<POSContext> uow)
        {
            _SubdivisionRepository = SubdivisionRepository;
            _logger = logger;
            _mapper = mapper;
            _uow = uow;
        }

        /// <summary>
        /// Handles the DeleteSubdivisionCommand to delete a Subdivision.
        /// </summary>
        /// <param name="request">The DeleteSubdivisionCommand containing Subdivision ID to be deleted.</param>
        /// <param name="cancellationToken">A cancellation token.</param>
        public async Task<ServiceResponse<bool>> Handle(DeleteSubdivisionCommand request, CancellationToken cancellationToken)
        {
            string errorMessage = null;
            try
            {
                // Check if the Subdivision entity with the specified ID exists
                var existingSubdivision = await _SubdivisionRepository.AllIncluding(c => c.Towns, cy => cy.Division).FirstOrDefaultAsync(c => c.Id == request.Id);
                if (existingSubdivision == null)
                {
                    errorMessage = $"Subdivision with ID {request.Id} not found.";
                    _logger.LogError(errorMessage);
                    return ServiceResponse<bool>.Return404(errorMessage);
                }
                existingSubdivision.IsDeleted = true;
                _SubdivisionRepository.Update(existingSubdivision);
                if (await _uow.SaveAsync() <= 0)
                {
                    return ServiceResponse<bool>.Return500();
                }
                return ServiceResponse<bool>.ReturnResultWith200(true);
            }
            catch (Exception e)
            {
                errorMessage = $"Error occurred while deleting Subdivision: {e.Message}";

                // Log error and return 500 Internal Server Error response with error message
                _logger.LogError(errorMessage);
                return ServiceResponse<bool>.Return500(e);
            }
        }
    }

}
