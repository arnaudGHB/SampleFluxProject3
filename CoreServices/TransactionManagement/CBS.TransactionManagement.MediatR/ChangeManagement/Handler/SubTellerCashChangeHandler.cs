...
namespace CBS.TransactionManagement.MediatR.ChangeManagement.Handler
{
    ...
    /// <summary>
    /// Handler for managing sub-teller cash changes.
    /// </summary>
    public class SubTellerCashChangeHandler : IRequestHandler<SubTellerCashChangeCommand, ServiceResponse<CashChangeHistoryDto>>
    {
        private readonly ITellerRepository _tellerRepository;
        private readonly ILogger<SubTellerCashChangeHandler> _logger;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="SubTellerCashChangeHandler"/> class.
        /// </summary>
        ...
        public SubTellerCashChangeHandler(
            ITellerRepository tellerRepository,
            ILogger<SubTellerCashChangeHandler> logger,
            IMapper mapper,
        ...
        /// <summary>
        /// Handles the command for managing sub-teller cash changes.
        /// </summary>
        ...
        public async Task<ServiceResponse<CashChangeHistoryDto>> Handle(SubTellerCashChangeCommand request, CancellationToken cancellationToken)
        {
            string reference = CurrencyNotesMapper.GenerateTransactionReference(_userInfoToken, LogAction.CashChangeOperationSubTill.ToString(), false);
            try
            {
                // Log the initiation of the change management process
                string startMessage = "Starting sub-teller cash change management.";
                ...
                // Perform the cash change operation via the teller repository
                var cashChangeHistory = await _subTellerProvisioningHistoryRepository.ManageCashChangeAsync(changeManagement);
                ...
                string successMessage = "Sub-teller cash change management completed successfully.";
            ...
            catch (Exception ex)
            {
                ...
                string errorMessage = $"An error occurred while managing sub-teller cash change: {ex.Message}";
