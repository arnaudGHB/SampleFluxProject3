...
namespace CBS.TransactionManagement.Handlers
{
    ...
    /// </summary>
    public class GetAccountByTellerIdQueryHandler : IRequestHandler<GetAccountByTellerIdQuery, ServiceResponse<AccountDto>>
    {
        ...
        public GetAccountByTellerIdQueryHandler(
        ...
        public async Task<ServiceResponse<AccountDto>> Handle(GetAccountByTellerIdQuery request, CancellationToken cancellationToken)
        {
            string errorMessage = null;
            try
            {
                // Retrieve the Account entity with the specified ID from the repository
                var entity = await _AccountRepository.AllIncluding(x => x.TellerId == request.TellerId, a => a.Product.CashDepositParameters, b => b.Product.WithdrawalParameters, c => c.Product.ManagementFeeParameters, d => d.Product.EntryFeeParameters, e => e.Product.ReopenFeeParameters, f => f.Product.CloseFeeParameters, t => t.Product.TransferParameters, s => s.Product.TermDeposits).FirstOrDefaultAsync();
                ...
                if (entity != null)
                {
                    ...
                    await APICallHelper.AuditLogger(_userInfoToken.Email, LogAction.Read.ToString(), request, "Teller account returned successfully", LogLevelInfo.Information.ToString(), 200, _userInfoToken.Token);
