...
{
    /// <summary>
    /// Handles the command to update an Account based on InitializeAccountCommand.
    /// </summary>
    public class InitializeAccountCommandHandler : IRequestHandler<InitializeAccountCommand, ServiceResponse<AccountDto>>
    {
        private readonly IAccountRepository _AccountRepository; // Repository for accessing Account data.
        private readonly ILogger<InitializeAccountCommandHandler> _logger; // Logger for logging handler actions and errors.
        private readonly IMapper _mapper; // AutoMapper for object mapping.
        private readonly IUnitOfWork<TransactionContext> _uow;
        private readonly UserInfoToken _userInfoToken;

        /// <summary>
        /// Constructor for initializing the InitializeAccountCommandHandler.
        /// </summary>
        /// <param name="AccountRepository">Repository for Account data access.</param>
        /// <param name="logger">Logger for logging handler actions and errors.</param>
        /// <param name="mapper">AutoMapper for object mapping.</param>
        public InitializeAccountCommandHandler(
            IAccountRepository AccountRepository,
            UserInfoToken userInfoToken,
            ILogger<InitializeAccountCommandHandler> logger,
            IMapper mapper,
        ...
        /// <summary>
        /// Handles the InitializeAccountCommand to update an Account.
        /// </summary>
        /// <param name="request">The InitializeAccountCommand containing updated Account data.</param>
        /// <param name="cancellationToken">A cancellation token.</param>
        public async Task<ServiceResponse<AccountDto>> Handle(InitializeAccountCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    if (!existingAccount.Status.Equals("INPROGRESS"))
                    {
                        // Log and return a 400 Bad Request response with an error message
                        string errorMessage = $"{request.Id} has already been activated";
