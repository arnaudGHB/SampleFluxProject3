...
namespace CBS.TransactionManagement.MediatR.TellerP.Handlers
{
    ...
    /// </summary>
    public class GetSubTellerProvioningHistoryByUserIDQueryHandler : IRequestHandler<GetSubTellerProvioningHistoryByUserIDQuery, ServiceResponse<List<SubTellerProvioningHistoryDto>>>
    {
        ...
        private readonly ILogger<GetSubTellerProvioningHistoryByUserIDQueryHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public GetSubTellerProvioningHistoryByUserIDQueryHandler(
            ISubTellerProvisioningHistoryRepository SubTellerProvioningHistoryRepository,
            UserInfoToken userInfoToken,
            IMapper mapper,
            ILogger<GetSubTellerProvioningHistoryByUserIDQueryHandler> logger)
        {
        ...
        public async Task<ServiceResponse<List<SubTellerProvioningHistoryDto>>> Handle(GetSubTellerProvioningHistoryByUserIDQuery request, CancellationToken cancellationToken)
        {
            string errorMessage = null;
            try
            {
                // Retrieve the SubTellerProvioningHistory entity with the specified ID from the repository
                var entity = await _SubTellerProvioningHistoryRepository.FindBy(a => a.UserIdInChargeOfThisTeller == request.UserInchargeId).Include(x => x.Teller).ToListAsync();
                if (entity != null)
                {
