...
{
    /// <summary>
    /// Handles the retrieval of all SubTellerProvioningHistory based on the GetAllSubTellerProvioningHistoryQuery.
    /// </summary>
    public class GetAllSubTellerProvioningHistoryQueryHandler : IRequestHandler<GetAllSubTellerProvioningHistoryQuery, ServiceResponse<List<SubTellerProvioningHistoryDto>>>
    {
        ...
        private readonly ILogger<GetAllSubTellerProvioningHistoryQueryHandler> _logger; // Logger for logging handler actions and errors.
        private readonly UserInfoToken _userInfoToken;

        /// <summary>
        /// Constructor for initializing the GetAllSubTellerProvioningHistoryQueryHandler.
        /// </summary>
        ...
        public GetAllSubTellerProvioningHistoryQueryHandler(
            ISubTellerProvisioningHistoryRepository SubTellerProvioningHistoryRepository,
            UserInfoToken UserInfoToken,
            IMapper mapper, ILogger<GetAllSubTellerProvioningHistoryQueryHandler> logger)
        {
        ...
        /// <summary>
        /// Handles the GetAllSubTellerProvioningHistoryQuery to retrieve all SubTellerProvioningHistory.
        /// </summary>
        /// <param name="request">The GetAllSubTellerProvioningHistoryQuery containing query parameters.</param>
        /// <param name="cancellationToken">A cancellation token.</param>
        public async Task<ServiceResponse<List<SubTellerProvioningHistoryDto>>> Handle(GetAllSubTellerProvioningHistoryQuery request, CancellationToken cancellationToken)
        {
            try
            {
                // Retrieve all SubTellerProvioningHistory entities from the repository
                var entities = await _SubTellerProvioningHistoryRepository.AllIncluding(x => x.Teller).Include(x => x.Teller.Transactions).ToListAsync();
