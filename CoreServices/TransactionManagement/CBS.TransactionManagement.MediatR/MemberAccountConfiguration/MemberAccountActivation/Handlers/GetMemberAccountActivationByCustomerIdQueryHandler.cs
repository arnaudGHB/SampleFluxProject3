...
namespace CBS.TransactionManagement.MemberAccountConfiguration.Handlers
{
    ...
    /// </summary>
    public class GetMemberAccountActivationByCustomerIdQueryHandler : IRequestHandler<GetMemberAccountActivationByCustomerIdQuery, ServiceResponse<MemberAccountActivationDto>>
    {
        ...
        private readonly ILogger<GetMemberAccountActivationByCustomerIdQueryHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public GetMemberAccountActivationByCustomerIdQueryHandler(
            IMemberAccountActivationRepository MemberAccountActivationRepository,
            IMapper mapper,
            ILogger<GetMemberAccountActivationByCustomerIdQueryHandler> logger)
        {
        ...
        public async Task<ServiceResponse<MemberAccountActivationDto>> Handle(GetMemberAccountActivationByCustomerIdQuery request, CancellationToken cancellationToken)
        {
            string errorMessage = null;
            try
            {
                // Retrieve the MemberAccountActivation entity with the specified ID from the repository
                var entity = await _MemberAccountActivationRepository.FindBy(a => a.CustomerId == request.CustomerId).Include(a => a.Fee).FirstOrDefaultAsync();
                if (entity != null)
                {
