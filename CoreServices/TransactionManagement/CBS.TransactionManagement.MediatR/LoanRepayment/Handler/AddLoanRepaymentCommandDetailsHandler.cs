...
namespace CBS.TransactionManagement.LoanRepayment.Handlers
{
    ...
    /// </summary>
    public class AddLoanRepaymentCommandDetailsHandler : IRequestHandler<AddLoanRepaymentCommandDetails, ServiceResponse<RefundDto>>
    {
        private readonly ILogger<AddLoanRepaymentCommandDetailsHandler> _logger; // Logger for error tracking and auditing.
        ...
        public AddLoanRepaymentCommandDetailsHandler(
            UserInfoToken userInfoToken,
            ILogger<AddLoanRepaymentCommandDetailsHandler> logger,
        ...
        /// <summary>
        /// Handles the loan repayment process by sending a request to the Loan Repayment API.
        /// </summary>
        /// <param name="request">Loan repayment request containing repayment details.</param>
        /// <param name="cancellationToken">A cancellation token.</param>
        /// <returns>Service response containing refund details if successful, or an error response.</returns>
        public async Task<ServiceResponse<RefundDto>> Handle(AddLoanRepaymentCommandDetails request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var apiResponse = await APICallHelper.LoanRepayment<ServiceResponse<RefundDto>>(
                    ...
                    requestData,
                    _pathHelper.RefundURL
                );

                // Step 3: Check API response status
                if (apiResponse.StatusCode == 200)
                {
                    string successMessage = $"[SUCCESS] Loan repayment processed successfully for Loan ID: {request.LoanId}, Amount: {request.Amount}.";
                ...
                {
                    string failureMessage = $"[ERROR] Loan Repayment API call failed. Status Code: {apiResponse.StatusCode}.";
