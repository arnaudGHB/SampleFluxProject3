using AutoMapper;
using MediatR;
using Microsoft.Extensions.Logging;
using CBS.BudgetManagement.Repository;
using CBS.BudgetManagement.Data;
using CBS.BudgetManagement.Common;

using CBS.BudgetManagement.Helper;
using CBS.BudgetManagement.MediatR.Commands;
using System;
using System.Threading;
using System.Threading.Tasks;
using CBS.BudgetManagement.Domain;

namespace CBS.BudgetManagement.MediatR.Handlers
{
    /// <summary>
    /// Handles the command to update an FiscalYear based on UpdateFiscalYearCommand.
    /// </summary>
    public class UpdateFiscalYearCommandHandler : IRequestHandler<UpdateFiscalYearCommand, ServiceResponse<FiscalYearDto>>
    {
        private readonly IFiscalYearRepository _FiscalYearRepository; // Repository for accessing FiscalYear data.
        private readonly IMapper _mapper; // AutoMapper for object mapping.
        private readonly ILogger<UpdateFiscalYearCommandHandler> _logger; // Logger for logging handler actions and errors.
        private readonly IUnitOfWork<BudgetManagementContext> _uow; // Unit of Work for managing transactions.
        private readonly UserInfoToken _userInfoToken; // User information token for audit logging.

        /// <summary>
        /// Constructor for initializing the UpdateFiscalYearCommandHandler.
        /// </summary>
        /// <param name="FiscalYearRepository">Repository for FiscalYear data access.</param>
        /// <param name="mapper">AutoMapper for object mapping.</param>
        /// <param name="logger">Logger for logging handler actions and errors.</param>
        /// <param name="userInfoToken">User info token for audit logging.</param>
        /// <param name="uow">Unit of Work for managing transactions.</param>
        public UpdateFiscalYearCommandHandler(
            IFiscalYearRepository FiscalYearRepository,
            IMapper mapper,
            ILogger<UpdateFiscalYearCommandHandler> logger,
            UserInfoToken userInfoToken,
            IUnitOfWork<BudgetManagementContext> uow)
        {
            _FiscalYearRepository = FiscalYearRepository;
            _mapper = mapper;
            _logger = logger;
            _userInfoToken = userInfoToken;
            _uow = uow;
        }

        /// <summary>
        /// Handles the UpdateFiscalYearCommand to update an FiscalYear.
        /// </summary>
        /// <param name="request">The UpdateFiscalYearCommand containing updated FiscalYear data.</param>
        /// <param name="cancellationToken">A cancellation token.</param>
        public async Task<ServiceResponse<FiscalYearDto>> Handle(UpdateFiscalYearCommand request, CancellationToken cancellationToken)
        {
            try
            {
                // Step 1: Retrieve the FiscalYear entity to be updated from the repository
                var existingFiscalYear = await _FiscalYearRepository.FindAsync(request.Id);

                // Step 2: Check if the FiscalYear entity exists
                if (existingFiscalYear != null)
                {
                    // Step 3: Update FiscalYear entity properties with values from the request

                    // Step 4: Use the repository to update the existing FiscalYear entity
                    _FiscalYearRepository.Update(existingFiscalYear);

                    // Step 5: Save changes using Unit of Work
                    await _uow.SaveAsync();

                    // Step 6: Log success message
                    string msg = $"FiscalYear '{existingFiscalYear.EndDate.Date}' updated successfully.";
                    _logger.LogInformation(msg);

                    // Step 7: Audit log the update action
                    await APICallHelper.AuditLogger(_userInfoToken.Email, LogAction.Update.ToString(), request, msg, LogLevelInfo.Information.ToString(), 200, _userInfoToken.Token);

                    // Step 8: Map the updated entity to a DTO for the response
                    var FiscalYearDto = _mapper.Map<FiscalYearDto>(existingFiscalYear);

                    // Step 9: Return the updated FiscalYearDto with a 200 status code
                    return ServiceResponse<FiscalYearDto>.ReturnResultWith200(FiscalYearDto);
                }
                else
                {
                    // Step 10: If the FiscalYear entity was not found, log an error and return a 404 Not Found response with an error message
                    string errorMessage = $"FiscalYear '{request.EndDate.Date}' was not found.";
                    _logger.LogError(errorMessage);

                    // Step 11: Audit log the failed update attempt
                    await APICallHelper.AuditLogger(_userInfoToken.Email, LogAction.Update.ToString(), request, errorMessage, LogLevelInfo.Information.ToString(), 404, _userInfoToken.Token);

                    return ServiceResponse<FiscalYearDto>.Return404(errorMessage);
                }
            }
            catch (Exception ex)
            {
                // Step 12: Log error and return a 500 Internal Server Error response with an error message
                string errorMessage = $"Error occurred while updating FiscalYear: {ex.Message}";
                _logger.LogError(errorMessage);

                // Step 13: Audit log the error
                await APICallHelper.AuditLogger(_userInfoToken.Email, LogAction.Update.ToString(), request, errorMessage, LogLevelInfo.Error.ToString(), 500, _userInfoToken.Token);

                return ServiceResponse<FiscalYearDto>.Return500(errorMessage);
            }
        }
    }
}
