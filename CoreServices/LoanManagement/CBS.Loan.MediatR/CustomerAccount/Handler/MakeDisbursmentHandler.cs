...
{
    public class MakeDisbursmentCommandHandler : IRequestHandler<MakeDisbursmentCommand, ServiceResponse<bool>>
    {
        private readonly ILogger<MakeDisbursmentCommandHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public MakeDisbursmentCommandHandler(
            UserInfoToken userInfoToken,
            ILogger<MakeDisbursmentCommandHandler> logger,
        ...
        public async Task<ServiceResponse<bool>> Handle(MakeDisbursmentCommand request, CancellationToken cancellationToken)
        {
            try
            {
                
                string serializedRequest = JsonConvert.SerializeObject(request);
                var serviceResponse = await APICallHelper.PostRequest<ServiceResponse<bool>>(_pathHelper, _userInfoToken.Token, serializedRequest, _pathHelper.MakeDisburstmentUrl);
        ...
        private async Task LogAuditError(MakeDisbursmentCommand request, string errorMessage)
        {
