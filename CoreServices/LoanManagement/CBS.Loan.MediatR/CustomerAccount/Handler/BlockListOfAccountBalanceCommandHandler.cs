...
namespace CBS.NLoan.MediatR.CustomerP.Handler
{
    ...
    {
        private readonly ILogger<BlockListOfAccountBalanceCommandHandler> _logger; // Logger for logging handler actions and errors.
        ...
        public async Task<ServiceResponse<bool>> Handle(BlockListOfAccountBalanceCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var serviceResponse = await APICallHelper.PostRequest<ServiceResponse<bool>>(_pathHelper, _userInfoToken.Token, serializedRequest, _pathHelper.BlockListOfAccountBalanceUrl);
            ...
            {
                // Log the error
                string errorMessage = $"Error occure while sending blocking accounts request: {e.Message}";
                ...
                return ServiceResponse<bool>.Return500("Transaction service returs an error on blocking accounts");
            }
        }

        private async Task LogAuditError(BlockListOfAccountBalanceCommand request, string errorMessage)
        {
