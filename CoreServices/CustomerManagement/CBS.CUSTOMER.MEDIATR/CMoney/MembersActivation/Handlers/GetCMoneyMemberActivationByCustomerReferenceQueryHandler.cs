...
namespace CBS.CUSTOMER.MEDIATR.CMoney.MembersActivation.Handlers
{
    ...
    /// </summary>
    public class GetCMoneyMemberActivationByCustomerReferenceQueryHandler : IRequestHandler<GetCMoneyMemberActivationByCustomerReferenceQuery, ServiceResponse<CMoneyMembersActivationAccountDto>>
    {
        ...
        private readonly ILogger<GetCMoneyMemberActivationByCustomerReferenceQueryHandler> _logger; // Logger for logging actions and errors.
        ...
        public GetCMoneyMemberActivationByCustomerReferenceQueryHandler(
            ICMoneyMembersActivationAccountRepository CMoneyMembersActivationAccountRepository,
            IMapper mapper,
            ILogger<GetCMoneyMemberActivationByCustomerReferenceQueryHandler> logger)
        {
        ...
        public async Task<ServiceResponse<CMoneyMembersActivationAccountDto>> Handle(GetCMoneyMemberActivationByCustomerReferenceQuery request, CancellationToken cancellationToken)
        {
            try
            {
                // Retrieve the member's activation account by CustomerReference
                var memberActivation =  _CMoneyMembersActivationAccountRepository.FindBy(x=> x.CustomerId==request.CustomerReference).FirstOrDefault();
                if (memberActivation == null)
                {
                    var message = $"C-MONEY member activation not found for CustomerReference {request.CustomerReference}.";
            ...
            catch (Exception e)
            {
                ...
                var errorMessage = $"Error occurred while retrieving activation for CustomerReference {request.CustomerReference}: {e.Message}";
