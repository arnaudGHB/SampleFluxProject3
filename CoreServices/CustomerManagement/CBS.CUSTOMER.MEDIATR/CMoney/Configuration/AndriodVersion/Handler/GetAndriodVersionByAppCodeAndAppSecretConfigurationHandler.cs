...
{
    /// <summary>
    /// Handles retrieval of Android version configuration by AppCodeAndAppSecret.
    /// Ensures structured logging, auditing, and error handling.
    /// </summary>
    public class GetAndriodVersionByAppCodeAndAppSecretConfigurationHandler : IRequestHandler<GetAndriodVersionByAppCodeAndAppSecretConfigurationQuery, ServiceResponse<AndriodVersionConfigurationDto>>
    {
        private readonly PathHelper _pathHelper;
        private readonly UserInfoToken _userInfoToken;
        private readonly IAndriodVersionConfigurationRepository _andriodVersionConfigurationRepository;
        private readonly IUnitOfWork<POSContext> _uow;
        private readonly ILogger<GetAndriodVersionByAppCodeAndAppSecretConfigurationHandler> _logger;
        private readonly IMapper _mapper;

        /// <summary>
        /// Initializes the handler for retrieving Android version configuration by AppCodeAndAppSecret.
        /// </summary>
        public GetAndriodVersionByAppCodeAndAppSecretConfigurationHandler(
            ILogger<GetAndriodVersionByAppCodeAndAppSecretConfigurationHandler> logger,
        ...
        /// <summary>
        /// Handles retrieval of Android version configuration based on AppCodeAndAppSecret.
        /// </summary>
        public async Task<ServiceResponse<AndriodVersionConfigurationDto>> Handle(GetAndriodVersionByAppCodeAndAppSecretConfigurationQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(request.AppCode) || string.IsNullOrWhiteSpace(request.AppSecret))
                {
                    string validationError = $"[ERROR] AppCode or  AppSecretCode  is missing in request. CorrelationId: '{correlationId}'";
                ...
                _logger.LogInformation($"[INFO] Retrieving Android version configuration for AppCode: '{request.AppCode}' and AppSecretCode: '{request.AppSecret}'. CorrelationId: '{correlationId}'");

                // 🔍 Step 2: Retrieve the Android version configuration
                var andriodVersionConfiguration =  _andriodVersionConfigurationRepository
                    .FindBy(x=>x.AppCode==request.AppCode && x.AppSecretcode==request.AppSecret).FirstOrDefault();

                if (andriodVersionConfiguration == null)    
                {
                    string notFoundMessage = $"[ERROR] Android version configuration not found for AppCode: '{request.AppCode}' and AppSecretCode: '{request.AppSecret}'. CorrelationId: '{correlationId}'";
