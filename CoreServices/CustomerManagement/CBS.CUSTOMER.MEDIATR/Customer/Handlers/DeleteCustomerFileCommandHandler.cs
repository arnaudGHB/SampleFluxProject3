...
namespace CBS.CUSTOMER.MEDIATR
{
    ...
    /// </summary>
    public class DeleteCustomerFileCommandHandler : IRequestHandler<DeleteCustomerFileCommand, ServiceResponse<List<CustomerFileMemberData>>>
    {
        ...
        public DeleteCustomerFileCommandHandler(
        ...
        public async Task<ServiceResponse<List<CustomerFileMemberData>>> Handle(DeleteCustomerFileCommand request, CancellationToken cancellationToken)
        {
            try
            {
        ...
        private  void DeleteGroupCustomer(CustomerFileMemberData data)
        {
            var groupCustomer= (_groupCustomerRepository.FindBy(u=>u.CustomerId==data.MemberNumber)).FirstOrDefault();
            if (groupCustomer != null)
            {
                _groupCustomerRepository.Remove(groupCustomer);
            }
           
        }

        private  void DeleteGroup(CustomerFileMemberData data)
        {
            var groupCustomer = (_groupRepository.FindBy(u => u.RegistrationNumber == data.MemberNumber)).FirstOrDefault();
            if (groupCustomer != null)
            {
                _groupRepository.Remove(groupCustomer);
            }

        }


        private  void DeleteCustomer(CustomerFileMemberData data)
        {
            var groupCustomer = (_customerRepository.FindBy(u => u.RegistrationNumber == data.MemberNumber)).FirstOrDefault();
            if (groupCustomer != null)
            {
                _customerRepository.Remove(groupCustomer);
            }
