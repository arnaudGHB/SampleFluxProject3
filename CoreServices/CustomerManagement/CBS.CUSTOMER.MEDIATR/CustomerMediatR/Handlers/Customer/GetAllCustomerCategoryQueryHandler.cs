...
namespace CBS.CUSTOMER.MEDIATR
{
    ...
    /// </summary>
    public class GetAllCustomerCategoryQueryHandler : IRequestHandler<GetAllCustomerCategoryQuery, ServiceResponse<List<GetAllCustomerCategory>>>
    {
        ...
        public async Task<ServiceResponse<List<GetAllCustomerCategory>>> Handle(GetAllCustomerCategoryQuery request, CancellationToken cancellationToken)
        {
            try
            {

                // Retrieve all CustomerCategory entities from the repository
                var entities = _mapper.Map< List<GetAllCustomerCategory>> (await _CustomerCategoryRepository.All.Where(x=>x.IsDeleted==false).ToListAsync());


                return ServiceResponse<List<GetAllCustomerCategory>>.ReturnResultWith200(entities);
            }
            catch (Exception e)
            {
                // Log error and return a 500 Internal Server Error response with error message
                _logger.LogError($"Failed to get all CustomerCategory: {e.Message}");
                return ServiceResponse<List<GetAllCustomerCategory>>.Return500(e, "Failed to get all CustomerCategory");
            }
        }
    }
}
