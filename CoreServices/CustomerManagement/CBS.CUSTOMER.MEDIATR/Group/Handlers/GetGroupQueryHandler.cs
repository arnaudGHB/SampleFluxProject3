...
namespace CBS.Group.MEDIATR.GroupMediatR.Handlers
{
    ...
    public class GetGroupQueryHandler : IRequestHandler<GetGroupQuery, ServiceResponse<CUSTOMER.DATA.Entity.Group>>
    {
        ...
        public async Task<ServiceResponse<CUSTOMER.DATA.Entity.Group>> Handle(GetGroupQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var entity = await _GroupRepository.AllIncluding(x => x.GroupDocuments, p => p.GroupType).Include(x => x.GroupCustomers).ThenInclude(gc => gc.Customer).FirstOrDefaultAsync(r => r.GroupId == request.GroupId && !r.IsDeleted);

                if (entity != null)
                {
                    return ServiceResponse<CUSTOMER.DATA.Entity.Group>.ReturnResultWith200(entity);
                }
                else
                {
