...
namespace CBS.CUSTOMER.HELPER.Helper
{
    ...
    public static class BaseUtilities
    {
        ...
        public static async Task LogAndAuditAsync(string message, object request, HttpStatusCodeEnum statusCode, LogAction logAction, LogLevelInfo logLevel,string corolationid=null)
        {
            // Call the AuditLogger method to record the action and associated details asynchronously.
            await APICallHelper.AuditLogger(logAction.ToString(), request, message, logLevel.ToString(), (int)statusCode, corolationid);
        }
        public static string PrependBaseUrl(string url, string baseUrl)
        {
            if (string.IsNullOrEmpty(url))
            {
                return url; // Return as is if URL is empty
            }

            if (string.IsNullOrEmpty(baseUrl))
            {
                return url; // Return as is if base URL is empty
            }

            // Remove all existing base URLs, keeping only the relative path
            url = Regex.Replace(url, @"(https?:\/\/[^/]+\/)+", "", RegexOptions.IgnoreCase).TrimStart('/');

            // Append the correct base URL
        ...
        /// <summary>
        /// Generates a unique Login ID using the Branch Code and the last 4 digits of the Member Reference Number.
        /// If a duplicate Login ID is detected, appends an incremental digit to ensure uniqueness.
        /// </summary>
        /// <param name="branchCode">The branch code of the member.</param>
        /// <param name="referenceNumber">The reference number of the member.</param>
        /// <param name="existingLoginIds">A collection of existing Login IDs to check for duplicates.</param>
        /// <returns>A unique Login ID.</returns>
        /// <exception cref="ArgumentException">Thrown if the reference number is invalid or less than 4 characters.</exception>
        /// <exception cref="InvalidOperationException">Thrown if a unique Login ID cannot be generated within reasonable attempts.</exception>
        public static string GenerateMemberLoginId(string branchCode, string referenceNumber, HashSet<string> existingLoginIds)
        {
            // Validate inputs
            ...
            {
                throw new ArgumentException("Reference number must be at least 4 characters long.", nameof(referenceNumber));
            }

            // Extract the last 4 digits of the reference number
            string lastFourDigits = referenceNumber[^4..];
            string baseLoginId = $"{branchCode}{lastFourDigits}";

            // Ensure uniqueness by appending a digit if a duplicate is found
            string uniqueLoginId = baseLoginId;
            int counter = 1;

            while (existingLoginIds.Contains(uniqueLoginId))
            {
                uniqueLoginId = $"{baseLoginId}{counter}";
                counter++;

                // Limit attempts to prevent infinite loops
                if (counter > 9999)
                {
                    throw new InvalidOperationException("Failed to generate a unique Login ID after multiple attempts.");
                }
            }
        ...
        public static DateTime UtcToLocal(this DateTime? dateTime)
        {
            ...
            return ConvertToDoualaTime(dateTime.Value);
        }
