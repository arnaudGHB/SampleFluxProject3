        ...
        public static void SendEmail(SendEmailSpecification sendEmailSpecification)
        {
            ...
            if (sendEmailSpecification.Attachments.Count > 0)
            {
                ...
                foreach (var file in sendEmailSpecification.Attachments)
                {
                    ...
                    int commaIndex = file.Src.IndexOf(',');
                    if (commaIndex == -1)
                    {
                    ...
                    string fileData = file.Src.Substring(commaIndex + 1); // Get the substring after the comma
                    byte[] bytes = Convert.FromBase64String(fileData);

                    using (var ms = new MemoryStream(bytes))
                    {
                        // Create the Attachment object directly from the MemoryStream
